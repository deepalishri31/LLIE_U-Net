...
checkpoint_dir = '/home/21i190003/DL_Project/Sony_Diff_Act/Sony_relu_ckpt/'
result_dir = '/home/21i190003/DL_Project/Sony_Diff_Act/Sony_relu_result/'
print('------------------------------------------------------------------\n------------------------------------------------------------------\n------------------------------------------------------------------')        
...
def upsample_and_concat(x1, x2, output_channels, in_channels):
    ...
    #print('Shape of upsampling and concatenation: ',deconv_output.shape)
...
def pack_raw(raw):
    ...
    H = img_shape[0]
    W = img_shape[1]
    #print('unpacked image: ',img_shape)

    out = np.concatenate((im[0:H:2, 0:W:2, :],
                          im[0:H:2, 1:W:2, :],
                          im[1:H:2, 1:W:2, :],
                          im[1:H:2, 0:W:2, :]), axis=2)
    #print('packed image shape: ',out.shape)
    return out
...
for epoch in range(150):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if input_images[str(ratio)[0:3]][ind] is None:
            #try:
                ...
                #print("packed and expanded image shape: ",input_images[str(ratio)[0:3]][ind].shape)
                gt_raw = rawpy.imread(gt_path)
                im = gt_raw.postprocess(use_camera_wb=True, half_size=False, no_auto_bright=True, output_bps=16)
                gt_images[ind] = np.expand_dims(np.float32(im / 65535.0), axis=0)
                #print('postprocesses gt image: ',gt_images[ind].shape)
        # crop
    ...
    mean_losses.append(np.mean(g_loss[np.where(g_loss)]))
    saver.save(sess, checkpoint_dir + 'model.ckpt')
    losses.append(G_current)
    mean_losses.append(np.mean(g_loss[np.where(g_loss)]))
np.savetxt(result_dir+'losses.txt',losses,delimiter = ',')
np.savetxt(result_dir+'mean_losses.txt',mean_losses,delimiter = ',')
